...
public class CutsceneManager {
    ...
    public void scene_skeletonLord() {
        ...
        if(scenePhase == 0) {
            ...
            for(int i = 0; i < gp.obj[1].length; i++) //Search a vacant slot for the iron door
            {
                if(gp.obj[gp.currentMap][i] == null)
                {
                    ...
                    gp.obj[gp.currentMap][i].worldY = gp.tileSize * 45;
            ...
            // Spawn dummy at player position
            for(int i = 0; i < gp.npc[1].length; i++) {
                if(gp.npc[gp.currentMap][i] == null) {
                    ...
                    gp.npc[gp.currentMap][i].drawing = true; // make sure dummy is drawn
                    gp.npc[gp.currentMap][i].collision = false; // don't block player
                    break;
                }
            }

            gp.player.drawing = false; // hide actual player
            scenePhase++;
        }
        if(scenePhase == 1) {
            gp.player.worldY -= 2;
        ...
        if(scenePhase == 4) {
            ...
            gp.stopMusic();
            gp.playMusic(20); // This starts the boss music!
        }
    }
    public void scene_ending()
    {
        ...
        if(scenePhase == 6)
        {
            ...
            drawString(alpha,120f, gp.screenHeight/2, "OOP Quest", 40);
